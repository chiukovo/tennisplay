<div v-if="view === 'events'" class="space-y-8">
    ...
    <div class="flex flex-nowrap items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-premium-title">開團揪球</h2>
            <p class="text-premium-subtitle">Join Tennis Events</p>
        </div>
        <div class="flex items-center gap-6">
            <button v-if="isLoggedIn && hasPlayerCard" @click="navigateTo('create-event')" class="bg-blue-600 text-white px-6 py-4 rounded-2xl font-bold uppercase tracking-widest text-xs shadow-2xl hover:-translate-y-1 hover:bg-blue-700 transition-all flex items-center gap-2 whitespace-nowrap shrink-0">
            ...
            <div class="text-right hidden sm:block">
                ...
                <div class="text-[11px] font-black text-slate-500 uppercase tracking-widest">場活動</div>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6 mb-10">
        ...
        <div class="flex flex-col lg:flex-row items-stretch lg:items-center gap-4 bg-white p-5 rounded-[32px] border border-slate-200 shadow-sm">
            ...
            <div class="shrink-0 flex items-center bg-slate-50 px-3 py-1 rounded-2xl border border-slate-100">
                ...
                <select v-model="eventRegionFilter" class="bg-transparent pl-2 pr-4 py-3 sm:py-3.5 focus:outline-none font-bold text-sm uppercase tracking-widest cursor-pointer appearance-none min-w-[100px] sm:min-w-[120px]">
            ...
            <div class="flex items-center gap-1 p-1 bg-slate-50 rounded-2xl overflow-x-auto no-scrollbar scroll-smooth">
                <button v-for="s in [
                    ...
                    :class="['px-4 sm:px-6 py-2.5 rounded-xl font-bold text-xs sm:text-sm uppercase tracking-widest transition-all whitespace-nowrap shrink-0', 
            ...
            <div class="flex items-center gap-2 sm:gap-3 flex-grow lg:flex-grow-0 bg-slate-50 px-3 sm:px-4 py-1 rounded-2xl border border-slate-100 min-h-[52px]">
                ...
                <div class="flex items-center gap-1 sm:gap-2 flex-1 min-w-0">
                    <input type="date" v-model="eventStartDate" @change="eventDateShortcut = 'custom'"
                        class="bg-transparent py-2.5 sm:py-3 font-bold text-[11px] sm:text-sm text-slate-700 outline-none w-0 flex-1 cursor-pointer min-w-0">
            ...
            <div class="relative flex flex-1 items-center gap-2">
                ...
                <button @click="handleEventSearch" class="px-5 sm:px-8 py-3.5 bg-slate-900 text-white rounded-2xl font-bold uppercase tracking-widest text-sm sm:text-base hover:bg-blue-600 transition-all shadow-lg active:scale-95 shrink-0">
                    搜尋
                </button>
            </div>
        </div>
    ...
    <div v-else-if="paginatedEvents.length > 0">
            ...
            <div v-for="event in paginatedEvents" :key="event.id" 
                ...
                <div class="flex items-center justify-between mb-5">
                    <div class="flex items-center gap-2">
                        <div class="flex items-center gap-1.5 bg-slate-50/80 px-3 py-1.5 rounded-full border border-slate-100">
                            ...
                            <span :class="['text-[11px] font-bold uppercase tracking-widest', event.status === 'open' ? 'text-green-600' : event.status === 'full' ? 'text-amber-600' : 'text-slate-400']">
                        ...
                        <div v-if="event.is_organizer" class="bg-blue-600 text-white px-2.5 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest flex items-center gap-1 shadow-lg shadow-blue-100">
                            <app-icon name="star" class-name="w-3.5 h-3.5"></app-icon>
                            主辦
                        </div>
                        <div v-else-if="event.has_joined" class="bg-green-600 text-white px-2.5 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest flex items-center gap-1 shadow-lg shadow-green-100">
                            <app-icon name="check" class-name="w-3.5 h-3.5"></app-icon>
                            已報名
                        </div>
                    </div>
                    <div class="flex items-center gap-1.5 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-100">
                        <span class="text-[11px] font-bold text-blue-500 uppercase tracking-tighter">缺</span>
                        <span class="text-lg font-black italic text-blue-600 leading-none">@{{ event.max_participants === 0 ? '∞' : event.spots_left }}</span>
                    </div>
                </div>

                {{-- Title --}}
                <h3 class="font-extrabold text-xl sm:text-2xl text-slate-900 group-hover:text-blue-600 transition-colors mb-6 leading-tight">
                    @{{ event.title }}
                </h3>
                ...
                <div class="space-y-3.5 mb-7">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <div class="text-[13px] font-bold text-slate-800">@{{ formatEventDate(event.event_date) }}</div>
                            <div class="text-[11px] font-medium text-slate-500 uppercase tracking-widest">活動時間</div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="text-[11px] font-medium text-slate-500 uppercase tracking-widest">區域地點</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-auto pt-6 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-2">
                        <template v-if="event.is_organizer">
                            <div class="flex items-center gap-2">
                                <button @click.stop="editEvent(event)" class="px-4 py-2 bg-blue-600 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 flex items-center gap-1.5">
                                    <app-icon name="edit" class-name="w-3.5 h-3.5"></app-icon>
                                    編輯
                                </button>
                                <button @click.stop="deleteEvent(event.id)" class="px-4 py-2 bg-white text-red-500 rounded-xl text-[11px] font-bold uppercase tracking-widest border border-red-100 hover:bg-red-50 transition-all flex items-center gap-1.5">
                        ...
                        <template v-else-if="event.has_joined">
                            <button @click.stop="leaveEvent(event.id)" class="px-5 py-2.5 bg-red-50 text-red-500 rounded-xl text-[11px] font-bold uppercase tracking-widest border border-red-100 hover:bg-red-500 hover:text-white transition-all shadow-lg shadow-red-100/50 flex items-center gap-2">
                                <app-icon name="x" class-name="w-3.5 h-3.5"></app-icon>
                                取消報名
                            </button>
                        </template>
                        <template v-else>
                            <button @click.stop="joinEvent(event.id)" class="px-5 py-2.5 bg-slate-900 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest hover:bg-blue-600 shadow-xl transition-all">
                                快速報名
                            </button>
                        </template>
                    </div>
                    <div class="text-right">
                        <div class="text-[11px] font-bold text-slate-500 uppercase tracking-widest leading-none mb-1">費用</div>
                ...
                <div class="mt-6 pt-5 border-t border-slate-50 flex items-center justify-between">
                    <div class="flex items-center gap-2.5">
                        ...
                        <span class="text-xs font-bold text-slate-600 truncate max-w-[100px]">@{{ event.player?.name || '主辦人' }}</span>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-1.5 text-slate-500">
                            <app-icon name="message-circle" class-name="w-3.5 h-3.5"></app-icon>
                            <span class="text-[11px] font-bold">@{{ eventComments[event.id]?.length || 0 }}</span>
                        </div>
                        <div class="flex items-center gap-1.5 text-slate-500">
                            <app-icon name="users" class-name="w-3.5 h-3.5"></app-icon>
                            <span class="text-[11px] font-bold">@{{ event.participants_count || 1 }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-if="eventTotalPages > 1" class="flex items-center justify-center gap-1 sm:gap-2 pt-12 pb-12 overflow-x-auto no-scrollbar flex-nowrap min-w-0 w-full">
            ...
            <button type="button" @click="eventCurrentPage = Math.max(1, eventsPagination.current_page - 1)" :disabled="eventsPagination.current_page === 1"
                :class="['w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-lg sm:rounded-xl font-black text-xs sm:text-sm transition-all', eventsPagination.current_page === 1 ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50']">
                ←
            </button>
            ...
            <div v-for="(page, idx) in eventDisplayPages" :key="'ep-' + idx" class="flex shrink-0">
                ...
                <button v-else type="button" @click="eventCurrentPage = page"
                    :class="['w-9 h-9 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl font-black text-xs sm:text-sm transition-all', eventsPagination.current_page === page ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50']">
                    @{{ page }}
                </button>
            </div>
            ...
            <button type="button" @click="eventCurrentPage = Math.min(eventTotalPages, eventsPagination.current_page + 1)" :disabled="eventsPagination.current_page === eventTotalPages"
                :class="['w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-lg sm:rounded-xl font-black text-xs sm:text-sm transition-all', eventsPagination.current_page === eventTotalPages ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50']">
                →
            </button>
        </div>
    </div>
    ...
    <div v-else class="text-center py-20">
        ...
        <p class="text-slate-500 font-bold mb-6">成為第一個開團的人吧！</p>
