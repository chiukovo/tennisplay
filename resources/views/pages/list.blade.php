...
<div v-if="view === 'list'" class="space-y-6">
    ...
    <div v-if="totalPages > 1" class="flex items-center justify-center gap-1 sm:gap-2 pt-8 pb-12 overflow-x-auto no-scrollbar flex-nowrap min-w-0 w-full">
        ...
        <button type="button" @click="currentPage = Math.max(1, playersPagination.current_page - 1)" :disabled="playersPagination.current_page === 1"
            :class="['w-8 h-8 sm:w-10 sm:h-10 shrink-0 rounded-lg sm:rounded-xl font-black text-xs sm:text-sm transition-all', playersPagination.current_page === 1 ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50']">
            ←
        </button>
        ...
        <div v-for="(page, idx) in displayPages" :key="'p-' + idx" class="flex shrink-0">
            ...
            <button v-else type="button" @click="currentPage = page"
                :class="['w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl font-black text-xs sm:text-sm transition-all', playersPagination.current_page === page ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50']">
                @{{ page }}
            </button>
        </div>
        ...
        <button type="button" @click="currentPage = Math.min(totalPages, playersPagination.current_page + 1)" :disabled="playersPagination.current_page === totalPages"
            :class="['w-8 h-8 sm:w-10 sm:h-10 shrink-0 rounded-lg sm:rounded-xl font-black text-xs sm:text-sm transition-all', playersPagination.current_page === totalPages ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50']">
            →
        </button>
    </div>
</div>
