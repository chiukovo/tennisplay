...
<div class="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-slate-950/90 backdrop-blur-2xl border border-white/10 rounded-[32px] p-2 flex justify-between items-center shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] z-[150] md:hidden">
    ...
    {{-- Profile / Auth --}}
    <a :href="isLoggedIn ? '/profile' : '/auth'" @click.prevent="isLoggedIn ? openProfile(currentUser.uid) : navigateTo('auth')" class="flex-1 flex flex-col items-center gap-1 py-2 rounded-2xl transition-all relative" :class="view === 'profile' || view === 'auth' ? 'text-blue-400' : 'text-slate-500'">
        <div class="relative">
