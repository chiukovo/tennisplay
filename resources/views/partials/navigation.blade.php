...
<nav class="bg-white/90 backdrop-blur-xl border-b sticky top-0 z-[100]">
    <div class="max-w-6xl mx-auto px-4 h-20 flex items-center justify-between gap-2">
        ...
        <div class="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            <a :href="hasPlayerCard ? '/events' : '/create'" @click.prevent="navigateTo(hasPlayerCard ? 'events' : 'create')" class="bg-slate-950 text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl text-[10px] sm:text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center gap-2 shadow-xl">
                ...
                <span class="sm:hidden">@{{ hasPlayerCard ? '揪團' : '製作' }}</span>
            </a>
        </div>
    </div>
</nav>
