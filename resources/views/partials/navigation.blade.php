    ...
    <div class="max-w-6xl mx-auto px-4 h-20 flex items-center justify-between gap-2">
        ...
        <div class="hidden md:flex gap-10 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
            ...
            <a href="/messages" @click.prevent="navigateTo('messages')" :class="['relative', view === 'messages' ? 'text-blue-600 border-b-2 border-blue-600 pb-1' : 'hover:text-slate-900 transition-colors']">
                約打訊息
                <span v-if="navRefreshing && navRefreshView === 'messages'" class="ml-2 inline-block w-2.5 h-2.5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></span>
        ...
        <div class="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            {{-- NTRP Guide Button --}}
            <button @click="showNtrpGuide = true" class="flex items-center gap-1.5 px-3 py-2 sm:px-3.5 sm:py-2 rounded-xl text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 hover:bg-blue-100 transition-all border border-blue-100 shadow-sm shrink-0">
                <app-icon name="help" class-name="w-3.5 h-3.5"></app-icon>
                <span>NTRP</span>
            </button>
            
            <a :href="hasPlayerCard ? '/events' : '/create'" @click.prevent="navigateTo(hasPlayerCard ? 'events' : 'create')" class="bg-slate-950 text-white px-3.5 sm:px-5 py-2 sm:py-2 rounded-xl text-[10px] sm:text-[11px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center gap-1.5 shadow-xl">
                <app-icon :name="hasPlayerCard ? 'users' : 'plus'" class-name="w-3.5 h-3.5 sm:w-4 h-4"></app-icon>
