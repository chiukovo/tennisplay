    ...
    <div class="max-w-6xl mx-auto px-4 h-20 flex items-center justify-between gap-2">
        <a href="/" class="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div class="flex flex-col leading-none">
                ...
                <span class="hidden md:block text-[10px] sm:text-[12px] font-bold text-slate-500 tracking-[0.2em] uppercase">愛網球</span>
            </div>
        </a>
        
        <div class="hidden md:flex gap-10 text-sm font-black uppercase tracking-[0.2em] text-slate-500">
        ...
        <div class="flex items-center gap-1.5 sm:gap-4 shrink-0">
            ...
            <a v-if="!isLoggedIn" href="/auth" @click.prevent="navigateTo('auth')" class="flex items-center gap-2 bg-[#06C755] hover:bg-[#05b34c] text-white px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl text-[12px] sm:text-sm font-black uppercase tracking-widest transition-all shadow-lg shadow-green-500/20">
            ...
            {{-- NTRP Guide Button --}}
            <button @click="showNtrpGuide = true" class="flex items-center gap-1 sm:gap-1.5 px-2.5 py-2 sm:px-3.5 sm:py-2 rounded-xl text-[11px] sm:text-[12px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 hover:bg-blue-100 transition-all border border-blue-100 shadow-sm shrink-0">
                <app-icon name="help" class-name="w-3.5 h-3.5"></app-icon>
                <span>NTRP</span>
            </button>
            
            <a :href="hasPlayerCard ? '/events' : '/create'" @click.prevent="navigateTo(hasPlayerCard ? 'events' : 'create')" class="bg-slate-950 text-white px-3 sm:px-5 py-2 sm:py-2 rounded-xl text-[11px] sm:text-[12px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center gap-1 sm:gap-1.5 shadow-xl">
